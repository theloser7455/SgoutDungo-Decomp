function scr_player_Sjumpprep()
{
	...
	if (sprite_index == spr_superjumplighten || sprite_index == spr_superjumpright || sprite_index == spr_superjumpleft)
	{
	    if (!place_meeting(x, y + 1, obj_railh) && !place_meeting(x, y + 1, obj_railh2))
	        hsp = move * 2;
	    else if (place_meeting(x, y + 1, obj_railh))
	...
	if (character == "S")
	{
	    if (floor(image_index) == (image_number - 1) && sprite_index == spr_snick_superjumpprep)
	        sprite_index = spr_superjumplighten;
	...
	if (!key_up && (character == "S" || sprite_index == spr_superjumplighten || sprite_index == spr_superjumpleft || sprite_index == spr_superjumpright) && !scr_solid(x, y - 16) && !scr_solid(x, y - 32))
	{
	...
	if (!audio_is_playing(superjumpholdsound))
	    scr_soundeffect(superjumpholdsound);
